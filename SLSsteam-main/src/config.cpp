...
bool CConfig::loadSettings()
{
	...
	// Parse CloudSync configuration
	const auto cloudSyncNode = node["CloudSync"];
	if (cloudSyncNode)
	{
		try
		{
			CloudSyncConfig_t _cloudSync;
			_cloudSync.enabled = getSetting<bool>(cloudSyncNode, "Enabled", false);
			_cloudSync.webdavUrl = getSetting<std::string>(cloudSyncNode, "WebDAVUrl", "");
			_cloudSync.username = getSetting<std::string>(cloudSyncNode, "Username", "");
			_cloudSync.password = getSetting<std::string>(cloudSyncNode, "Password", "");

			// Parse AppIds list (if provided, sync only these)
			if (cloudSyncNode["AppIds"])
			{
				for (auto& appIdNode : cloudSyncNode["AppIds"])
				{
					try
					{
						_cloudSync.appIds.emplace(appIdNode.as<uint32_t>());
					}
					catch(...) {}
				}
			}

			cloudSync.set(_cloudSync);

			if (_cloudSync.enabled)
			{
				g_pLog->info("CloudSync enabled, WebDAV: %s\n", _cloudSync.webdavUrl.c_str());
				if (!_cloudSync.appIds.empty())
				{
					g_pLog->info("CloudSync limited to %zu apps\n", _cloudSync.appIds.size());
				}
			}
		}
		catch (...)
		{
			g_pLog->warn("Failed to parse CloudSync config!");
		}
	}
