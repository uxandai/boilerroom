...
// Stored appId for CloudSync (set when RemoteStorage is accessed)
static uint32_t g_currentCloudAppId = 0;

// CloudSync hook functions
static bool hkClientRemoteStorage_FileWrite(void* pClientRemoteStorage, uint32_t appId, const char* filename, const void* data, int32_t dataSize)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		g_pLog->debug("CloudSync: Intercepted FileWrite(%u, %s, %d bytes)\n", appId, filename, dataSize);
		if (CloudSync::fileWrite(appId, filename, data, dataSize))
		{
			return true;
		}
	}

	// Fall back to original Steam Cloud
	return Hooks::IClientRemoteStorage_FileWrite.originalFn.fn(pClientRemoteStorage, appId, filename, data, dataSize);
}

static int32_t hkClientRemoteStorage_FileRead(void* pClientRemoteStorage, uint32_t appId, const char* filename, void* buffer, int32_t bufferSize)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		g_pLog->debug("CloudSync: Intercepted FileRead(%u, %s, %d buffer)\n", appId, filename, bufferSize);
		int32_t result = CloudSync::fileRead(appId, filename, buffer, bufferSize);
		if (result >= 0)
		{
			return result;
		}
	...
	// Fall back to original Steam Cloud
	return Hooks::IClientRemoteStorage_FileRead.originalFn.fn(pClientRemoteStorage, appId, filename, buffer, bufferSize);
}

static bool hkClientRemoteStorage_FileDelete(void* pClientRemoteStorage, uint32_t appId, const char* filename)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		g_pLog->debug("CloudSync: Intercepted FileDelete(%u, %s)\n", appId, filename);
		CloudSync::fileDelete(appId, filename);
		// Also delete from Steam Cloud to stay in sync
	}

	return Hooks::IClientRemoteStorage_FileDelete.originalFn.fn(pClientRemoteStorage, appId, filename);
}

static bool hkClientRemoteStorage_FileExists(void* pClientRemoteStorage, uint32_t appId, const char* filename)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		bool exists = CloudSync::fileExists(appId, filename);
		if (exists)
		{
			return true;
		}
	...
	return Hooks::IClientRemoteStorage_FileExists.originalFn.fn(pClientRemoteStorage, appId, filename);
}

static int32_t hkClientRemoteStorage_GetFileSize(void* pClientRemoteStorage, uint32_t appId, const char* filename)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		int32_t size = CloudSync::getFileSize(appId, filename);
		if (size >= 0)
		{
			return size;
		}
	...
	return Hooks::IClientRemoteStorage_GetFileSize.originalFn.fn(pClientRemoteStorage, appId, filename);
}

static int64_t hkClientRemoteStorage_GetFileTimestamp(void* pClientRemoteStorage, uint32_t appId, const char* filename)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		int64_t ts = CloudSync::getFileTimestamp(appId, filename);
		if (ts > 0)
		{
			return ts;
		}
	...
	return Hooks::IClientRemoteStorage_GetFileTimestamp.originalFn.fn(pClientRemoteStorage, appId, filename);
}

static int32_t hkClientRemoteStorage_GetFileCount(void* pClientRemoteStorage, uint32_t appId)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		return CloudSync::getFileCount(appId);
	}

	return Hooks::IClientRemoteStorage_GetFileCount.originalFn.fn(pClientRemoteStorage, appId);
}

static const char* hkClientRemoteStorage_GetFileNameAndSize(void* pClientRemoteStorage, uint32_t appId, int index, int32_t* outSize)
{
	g_currentCloudAppId = appId;

	if (CloudSync::shouldHandle(appId))
	{
		const char* name = CloudSync::getFileNameAndSize(appId, index, outSize);
		if (name != nullptr)
		{
			return name;
		}
	...
	return Hooks::IClientRemoteStorage_GetFileNameAndSize.originalFn.fn(pClientRemoteStorage, appId, index, outSize);
}
...
static void hkClientRemoteStorage_PipeLoop(void* pClientRemoteStorage, void* a1, void* a2, void* a3)
{
	...
	if (!hooked)
	{
		...
		// CloudSync hooks
		if (g_config.cloudSync.get().enabled)
		{
			g_pLog->info("CloudSync: Setting up RemoteStorage hooks\n");

			Hooks::IClientRemoteStorage_FileWrite.setup(vft, VFTIndexes::IClientRemoteStorage::FileWrite, hkClientRemoteStorage_FileWrite);
			Hooks::IClientRemoteStorage_FileWrite.place();

			Hooks::IClientRemoteStorage_FileRead.setup(vft, VFTIndexes::IClientRemoteStorage::FileRead, hkClientRemoteStorage_FileRead);
			Hooks::IClientRemoteStorage_FileRead.place();

			Hooks::IClientRemoteStorage_FileDelete.setup(vft, VFTIndexes::IClientRemoteStorage::FileDelete, hkClientRemoteStorage_FileDelete);
			Hooks::IClientRemoteStorage_FileDelete.place();

			Hooks::IClientRemoteStorage_FileExists.setup(vft, VFTIndexes::IClientRemoteStorage::FileExists, hkClientRemoteStorage_FileExists);
			Hooks::IClientRemoteStorage_FileExists.place();

			Hooks::IClientRemoteStorage_GetFileSize.setup(vft, VFTIndexes::IClientRemoteStorage::GetFileSize, hkClientRemoteStorage_GetFileSize);
			Hooks::IClientRemoteStorage_GetFileSize.place();

			Hooks::IClientRemoteStorage_GetFileTimestamp.setup(vft, VFTIndexes::IClientRemoteStorage::GetFileTimestamp, hkClientRemoteStorage_GetFileTimestamp);
			Hooks::IClientRemoteStorage_GetFileTimestamp.place();

			Hooks::IClientRemoteStorage_GetFileCount.setup(vft, VFTIndexes::IClientRemoteStorage::GetFileCount, hkClientRemoteStorage_GetFileCount);
			Hooks::IClientRemoteStorage_GetFileCount.place();

			Hooks::IClientRemoteStorage_GetFileNameAndSize.setup(vft, VFTIndexes::IClientRemoteStorage::GetFileNameAndSize, hkClientRemoteStorage_GetFileNameAndSize);
...
namespace Hooks
{
	...
	// CloudSync hooks
	VFTHook<IClientRemoteStorage_FileWrite_t> IClientRemoteStorage_FileWrite("IClientRemoteStorage::FileWrite");
	VFTHook<IClientRemoteStorage_FileRead_t> IClientRemoteStorage_FileRead("IClientRemoteStorage::FileRead");
	VFTHook<IClientRemoteStorage_FileDelete_t> IClientRemoteStorage_FileDelete("IClientRemoteStorage::FileDelete");
	VFTHook<IClientRemoteStorage_FileExists_t> IClientRemoteStorage_FileExists("IClientRemoteStorage::FileExists");
	VFTHook<IClientRemoteStorage_GetFileSize_t> IClientRemoteStorage_GetFileSize("IClientRemoteStorage::GetFileSize");
	VFTHook<IClientRemoteStorage_GetFileTimestamp_t> IClientRemoteStorage_GetFileTimestamp("IClientRemoteStorage::GetFileTimestamp");
	VFTHook<IClientRemoteStorage_GetFileCount_t> IClientRemoteStorage_GetFileCount("IClientRemoteStorage::GetFileCount");
	VFTHook<IClientRemoteStorage_GetFileNameAndSize_t> IClientRemoteStorage_GetFileNameAndSize("IClientRemoteStorage::GetFileNameAndSize");
