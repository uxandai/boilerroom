...
public:
	...
	// CloudSync configuration for WebDAV-based cloud save replacement
	struct CloudSyncConfig_t
	{
		bool enabled = false;
		std::string webdavUrl;      // e.g., "https://nextcloud.example.com/remote.php/dav/files/user/"
		std::string username;
		std::string password;
		std::unordered_set<uint32_t> appIds;  // Empty = sync all, otherwise only listed apps
	};
	...
	// Cloud sync to WebDAV
	MTVariable<CloudSyncConfig_t> cloudSync;
