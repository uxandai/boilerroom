    ...
    return (
        <Card className="bg-[#1b2838] border-[#2a475e]">
            <CardHeader className="pb-3 cursor-pointer select-none" onClick={() => setIsExpanded(!isExpanded)}>
                ...
                <CardDescription>System dependencies and SLSteam status.</CardDescription>
            </CardHeader>
            ...
            {isExpanded && (
                <CardContent className="space-y-6 pt-0 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="space-y-2 pt-4 border-t border-[#2a475e]">
                        <div className="flex justify-between items-center text-sm font-medium text-gray-300">
                            <span className="flex items-center gap-2"><Trash2 className="w-4 h-4" /> Manifest Cache</span>
                        </div>
                        <div className="bg-[#0a0a0a] p-3 rounded border border-[#2a475e] flex justify-between items-center">
                            <div className="text-xs text-gray-400">
                                Cached game manifests (faster reinstalls)
                            </div>
                            <Button
                                size="sm"
                                variant="destructive"
                                ...
                                onClick={async () => {
                                    try {
                                        const { clearManifestCache } = await import("@/lib/api");
                                        const count = await clearManifestCache();
                                        addLog("info", `Cleared ${count} cached manifests`);
                                    } catch (e) {
                                        addLog("error", `Failed to clear cache: ${e}`);
                                    }
                                }}
                            >
                                Clear Cache
