...
import { getSteamLibraries, startPipelinedInstall, downloadBundle, extractManifestZip, checkGameInstalled, installDepotKeysOnly, getCachedManifest, cacheManifest, type DepotInfo, type InstalledDepot, type DepotKeyInfo } from "@/lib/api";
...
export function InstallModal({ isOpen, onClose, game, preExtractedZipPath }: InstallModalProps) {
  ...
  const [usedCache, setUsedCache] = useState(false); // Track if manifest came from cache
  ...
  const loadDepots = async () => {
    ...
    try {
      ...
      // Step 2: Get manifest - either from pre-extracted path, cache, or download from API
      let zipPath: string;
      let fromCache = false;
      if (preExtractedZipPath) {
        ...
        // Check cache first
        const cached = await getCachedManifest(game.game_id);
        if (cached) {
          zipPath = cached;
          fromCache = true;
          addLog("info", `Using cached manifest: ${zipPath}`);
        } else {
          // Download from Depot Provider API
          addLog("info", `Downloading manifest for ${game.game_name} (${game.game_id})...`);
          ...
          try {
            await cacheManifest(game.game_id, zipPath);
            addLog("info", `Manifest cached for future use`);
          } catch (cacheErr) {
            addLog("warn", `Failed to cache manifest: ${cacheErr}`);
          }
        }
      }
          ...
          <div className="flex gap-2">
            ...
            >
              {isInstalling ? (
                ...
                <>
                  <Download className="w-4 h-4 mr-2" />
                  Install ({selectedCount}){usedCache && " 💾"}
                </>
              )}
            </Button>
          </div>
