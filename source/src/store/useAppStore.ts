...
export interface Settings {
  ...
  achievementMethod: "web_api" | "steam_cm"; // SLSah (web_api) vs SLScheevo (steam_cm)
  steamUsername: string; // Steam account username (for CM method)
  steamPassword: string; // Steam password (for CM method, not stored permanently)
...
interface AppState {
  ...
  steamCmLoggedIn: boolean; // Steam CM login status (persists across tab switches)
  ...
  setSteamCmLoggedIn: (loggedIn: boolean) => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  setSteamCmLoggedIn: (loggedIn) => set({ steamCmLoggedIn: loggedIn }),
