...
/// Detect if app is a soundtrack/music app based on name
fn is_soundtrack(name: &str) -> bool {
    let name_upper = name.to_uppercase();
    name_upper.contains("OST") || name_upper.contains("SOUNDTRACK")
}
...
impl InstallManager {
    ...
    pub fn start_pipeline(
        ...
        // Detect if this is a soundtrack (OST/SOUNDTRACK in name)
        let is_soundtrack_app = is_soundtrack(&game_name);
        if is_soundtrack_app {
            eprintln!("[Soundtrack] Detected soundtrack: {}", game_name);
        }
        
        // For soundtracks: use steamapps/music/ instead of steamapps/common/
        // target_directory is typically steamapps/common, we need steamapps/music
        let actual_target_dir = if is_soundtrack_app {
            PathBuf::from(&target_directory)
                .parent()
                .map(|p| p.join("music"))
                .unwrap_or_else(|| PathBuf::from(&target_directory))
                .to_string_lossy()
                .to_string()
        } else {
        ...
        let download_dir = if is_local {
            let path = PathBuf::from(&actual_target_dir).join(&folder_name);
            std::fs::create_dir_all(&path).map_err(|e| format!("Failed to create {} directory: {}", if is_soundtrack_app { "music" } else { "game" }, e))?;
            path
        } else {
        ...
        let target_dir = actual_target_dir.clone(); // Use music/ or common/ based on soundtrack detection
        let download_dir_clone = download_dir.clone();
        let depot_keys_clone = depot_keys.clone();
        let app_token_clone = app_token.clone();
        let is_soundtrack_clone = is_soundtrack_app;
        ...
        thread::spawn(move || {
            ...
            // ========================================
            // PHASE 2a: SOUNDTRACK FILE FILTERING
            // ========================================
            // For soundtracks, keep only audio and image files, remove everything else
            if is_soundtrack_clone {
                m.update_status("configuring", "Filtering soundtrack files...");
                eprintln!("[Soundtrack] Filtering files in {:?}", download_dir_clone);
                
                let allowed_extensions = ["mp3", "flac", "wav", "ogg", "m4a", "jpg", "jpeg", "png", "gif", "bmp"];
                let mut files_to_delete: Vec<PathBuf> = Vec::new();
                let mut audio_files: Vec<PathBuf> = Vec::new();
                
                // First pass: identify files to keep and delete
                for entry in WalkDir::new(&download_dir_clone).into_iter().filter_map(|e| e.ok()) {
                    let path = entry.path();
                    if path.is_file() {
                        ...
                        if allowed_extensions.contains(&ext.as_str()) {
                            ...
                            eprintln!("[Soundtrack] Will delete non-audio file: {:?}", path.file_name());
                        }
                    }
                }
                ...
                for file in &files_to_delete {
                    if let Err(e) = std::fs::remove_file(file) {
                        eprintln!("[Soundtrack] Failed to delete {:?}: {}", file, e);
                    }
                }
                eprintln!("[Soundtrack] Deleted {} non-audio files, keeping {} audio/image files", 
                    files_to_delete.len(), audio_files.len());
                
                // Move audio files from subfolders to root
                for file in &audio_files {
                    if let Some(parent) = file.parent() {
                        if parent != download_dir_clone {
                            // File is in a subfolder, move to root
                            if let Some(filename) = file.file_name() {
                                let dest = download_dir_clone.join(filename);
                                if !dest.exists() {
                                    if let Err(e) = std::fs::rename(file, &dest) {
                                        eprintln!("[Soundtrack] Failed to move {:?}: {}", filename, e);
                                    } else {
                                        eprintln!("[Soundtrack] Moved {:?} to root", filename);
                                    }
                                }
                            }
                        }
                ...
                for entry in WalkDir::new(&download_dir_clone).contents_first(true).into_iter().filter_map(|e| e.ok()) {
                    let path = entry.path();
                    if path.is_dir() && path != download_dir_clone {
            ...
            // Build InstalledDepots section from actual depots
            let installed_depots_str = depots.iter()
...
{installed_depots}
                ...
                installed_depots = installed_depots_str
            );
