...
/// Routes to Web API (SLSah) or Steam CM (SLScheevo) based on settings
#[tauri::command]
pub async fn generate_achievements(
    ...
    if steam_user_id.is_empty() {
        return Err("Steam User ID is required. Configure it in Settings".to_string());
    }
    ...
    if method == "steam_cm" {
        // Steam CM method (SLScheevo) - read credentials from store
        let tool_settings = store.get("toolSettings").ok_or(
            "Steam CM login required. Go to Settings → Steam Achievements and login first.",
        )?;

        let steam_username = tool_settings
            .get("steamUsername")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let steam_password = tool_settings
            .get("steamPassword")
            .and_then(|v| v.as_str())
            .unwrap_or("");

        if steam_username.is_empty() || steam_password.is_empty() {
            return Err("Steam username and password required for CM method. Configure in Settings → Steam Achievements.".to_string());
        }

        // Login to Steam (uses guard data if previously cached, otherwise waits for mobile approval)
        let connection = crate::steam_cm::steam_login(steam_username, steam_password).await?;

        // Fetch schema using the connection
        let result = crate::steam_cm::fetch_schema_with_connection(&connection, &app_id).await?;
        ...
        // Web API method (SLSah) - default
        if steam_api_key.is_empty() {
            return Err(
                "Steam API Key is required for Web API method. Configure it in Settings"
                    .to_string(),
            );
        }
...
/// Logs into Steam, fetches schema, then logs out
/// User must approve on Steam mobile app when prompted
#[tauri::command]
pub async fn generate_achievements_cm(
    app_id: String,
    steam_username: String,
    steam_password: String,
) -> Result<String, String> {
    if steam_username.is_empty() || steam_password.is_empty() {
        return Err("Steam username and password are required for CM method".to_string());
    }

    // Login to Steam (will wait for mobile app approval)
    let connection = crate::steam_cm::steam_login(&steam_username, &steam_password).await?;

    // Fetch schema using the connection
    let result = crate::steam_cm::fetch_schema_with_connection(&connection, &app_id).await?;

    // Connection will be dropped here, logging out
...
/// Login to Steam using CM protocol
/// User must approve on Steam mobile app when prompted
/// Returns success if login completed
#[tauri::command]
pub async fn steam_cm_login(
    steam_username: String,
    steam_password: String,
) -> Result<String, String> {
    if steam_username.is_empty() || steam_password.is_empty() {
        return Err("Steam username and password are required".to_string());
    }

    // Login to Steam (will wait for mobile app approval)
    let _connection = crate::steam_cm::steam_login(&steam_username, &steam_password).await?;

    // Connection established successfully
    // Note: Connection is dropped here, but steam_vent caches the guard data
    // so subsequent logins will be faster

    Ok("Steam login successful! Guard data cached for future use.".to_string())
}
