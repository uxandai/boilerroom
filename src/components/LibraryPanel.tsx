...
export function LibraryPanel() {
  ...
  // Handler for copying to remote
  const handleCopy = useCallback((game: InstalledGame) => {
    setCopyToRemoteGame(game);
  }, [setCopyToRemoteGame]);

  // Handler for uninstallation
  ...
  }, [setSelectedGameForCard]);

  // Memoize filtered games to prevent re-filtering on every render
            ...
            <div className="space-y-2">
              {filteredGames.map((game) => (
                <LibraryGameItem
                  ...
                  showCopyButton={connectionMode === "local"}
                />
              ))}
            </div>
          )}
